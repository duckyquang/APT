...
export function Today(p: { userId: string; profile: Profile; version: number; onChange: () => void }) {
  ...
  useEffect(() => {
    ...
    Promise.all([dayTotals(date), dayMeals(date), progressPhotoPath(date), latestWeight(), latestPlan('meal'), rangeTotals(days[0], date)])
      .then(async ([t, m, path, w, mp, r]) => {
  ...
  const macro = (label: string, value: number, t?: number) => (
    <div className="macro">
      <span>{label}</span>
  ...
  return (
    <div className="grid">
      ...
      <section className="tile">
        ...
        {macro('Protein', totals.protein_g, target.protein_g)}
        {macro('Carbs', totals.carbs_g, target.carbs_g)}
        {macro('Fat', totals.fat_g, target.fat_g)}
      </section>

      <section className="tile">
        <Label meta={`target ${target.water_ml ?? 2500} ml`}>Water</Label>
      ...
      <section className="tile span">
        ...
        {meals.length === 0 && <p className="muted" style={{ marginTop: 12 }}>Nothing logged yet.</p>}
        {meals.map(m => (
        ...
        {planned.length > 0 && (
          <>
            <div className="label" style={{ marginTop: 14 }}>Planned for today</div>
            {planned.map((m, i) => (
      ...
      <section className="tile">
        <Label meta={progressUrl ? 'done today' : 'not yet'}>Progress photo</Label>
