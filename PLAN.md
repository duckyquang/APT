...
2. **Anthropic is the only provider at launch.** Default model `claude-opus-5`, with `claude-sonnet-5` and `claude-haiku-4-5` in a dropdown. One SDK, one code path. If a user asks for a second provider it's Gemini via `@google/genai`: no browser-unlock flag, CORS reflects any origin, about 40 lines. OpenAI works too (`dangerouslyAllowBrowser`, Responses API) but nobody has asked. Never Gemini's OpenAI-compat endpoint, its preflight returns 403.
...
Black ground, white text, one white accent, hairline borders. Apple-like means restraint, not glass.
